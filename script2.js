function carregar(){
    ...
    var n3 =  Number(prompt("Digite a 3° nota: "))
    ...
    if (media >=6){
        ...
        msg.innerHTML = `Sua média é <strong>${media}</strong> e você está: <strong>APROVADO!</strong>`
        document.body.style.background = "green"
    }else if (media >=5 && media <=5.99) {
        img.src = 'imagens/emojineutro2.png'
        msg.innerHTML = `Sua média é <strong>${media}</strong> e você está de <strong>recuperação!</strong>`
        document.body.style.background = "#ffcf21"
    } else{
        img.src = 'imagens/emojitriste2.png'
        msg.innerHTML = `Sua média é <strong>${media}</strong> e você está: <strong>REPROVADO!</strong>`
